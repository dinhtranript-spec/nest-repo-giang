...
export class MemberClassService {
    private readonly logger = new Logger(MemberClassService.name);
    ...
    /**
     * Lấy danh sách tất cả member classes
     */
    async getAllMemberClasses(user: User) {
        const url = `${this.ssConfigUrl}/member-classes`;
    ...
    /**
     * Lấy thông tin member class theo ID
     */
    async getMemberClassById(user: User, id: string) {
        const url = `${this.ssConfigUrl}/member-classes/${id}`;
        const response = await this.makeRequest('GET', url);
